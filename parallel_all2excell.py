...
class Darknet(nn.Module):
    ...
    def load_weights(self, weightfile, every_launch, ic_bins, oc_bins, \
        ...
        # create excel  
        filefullpath = 'output/launch_all_' + str(every_launch) + '.xlsx'
        ...
        ws.cell(row=1, column=3, value='Ori Sparsity')
        ws.cell(row=1, column=4, value='Launch')
        ws.cell(row=1, column=5, value='Full')
        ws.cell(row=1, column=6, value='Ratio')
        ...
        for i in range(len(self.module_list)):
            ...
            if module_type == "convolutional" or module_type == "dilated_convolutional":#dilated_convolutional for deeplab
                ...
                if conv_heights == 3 and i != 0:
                    ...
                    #     str(sparsity).ljust(10)       + \
                    #     str(total_common).ljust(10, ' ')    + \
                    #     str(num_full_launch).ljust(10, ' ') + \
                    #     str(total_ratio).ljust(10, ' ')     + '\n'
                    # print(s)

                    ws.cell(row=row_count, column=1, value=str(i))
                    ws.cell(row=row_count, column=2, value=str(str_size))
                    ws.cell(row=row_count, column=3, value=str(sparsity))
                    ws.cell(row=row_count, column=4, value=str(total_common))
                    ws.cell(row=row_count, column=5, value=str(num_full_launch))
                    ws.cell(row=row_count, column=6, value=str(total_ratio))
                    row_count += 1

                    print('Dump Layer' + str(i) + ' Done!')

        wb.save(filefullpath) 

# python parallel_all2excell.py
if __name__ == "__main__":
    '''
    all layer kernel size is 3*3

    '''
    net_cfg = Darknet('/home/aiden00/abwu_workspace/yolov4best/yolov4.cfg')
    net_cfg.load_weights("/home/aiden00/abwu_workspace/yolov4best/yolov4.best.0", every_launch=4, oc_bins=16, ic_bins=4)  
